...
layui.define(['laytpl', 'laypage', 'layer', 'form', 'util'], function(exports){
    ...
    var $ = layui.$
        ...
        //字符常量
        ,MOD_NAME = 'laytable', ELEM = '.layui-table', THIS = 'layui-this', SHOW = 'layui-show', HIDE = 'layui-hide', DISABLED = 'layui-disabled', NONE = 'layui-none'
    ...
    Class.prototype.render = function(){
        ...
        if(typeof options.page === 'object'){
            ...
            if (options.where.page){
                that.page = options.page.curr = options.where.page;
            }else {
    ...
    Class.prototype.pullData = function(curr){
        ...
        if(options.url){ //Ajax请求
            ...
            if (setter)
            {
                ...
                options.headers[setter.request.tokenName] = setter.request.tokenName
                in options.headers
                    ? options.headers[setter.request.tokenName]
                    : (layui.data(setter.tableName)[setter.request.tokenName] || '');
                options.headers[setter.request.refreshTokenName] = setter.request.refreshTokenName
                in options.headers
                    ? options.headers[setter.request.refreshTokenName]
                    : (layui.data(setter.tableName)[setter.request.refreshTokenName] || '');
            }
            ...
            $.ajax({
                ...
                ,error: function(e, msg){
                    ...
                    if (responseJSON && responseJSON.status == setter.response.statusCode.logout)
                    {
                        layui.view && layui.view.exit();
                    }
